...
class module extends jpage
{
  ...
  private String Module_LoadQuote()
  {
    ...
    if (tArys != null)
    {
      ...
      {
        tmpstr += "[quote]";
        tmpstr += "[b]" + encode.htmlencode(cls.toString(tDbc.getValue(tAry, cls.cfnames(tfpreT, "author")))) + " " + conf.jt.itake("default.txt-release", "lng") + " " + encode.htmlencode(cls.formatDate(cls.toString(tDbc.getValue(tAry, cls.cfnames(tfpreT, "time"))))) + "[/b][br]";
        tmpstr += encode.htmlencode(conf.common.repathdecode(cls.toString(tDbc.getValue(tAry, cls.cfnames(tfpreT, "content")))));
        tmpstr += "[/quote][br]";
      }
    }
    tmpstr = conf.ajaxPreContent + tmpstr;
