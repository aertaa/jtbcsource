...
class module extends jpage
{
  ...
  private String PP_FormatURLs(String argurls)
  {
    String tmpstr = "";
    String turls = argurls;
    if (!cls.isEmpty(turls))
    {
      String[] turlsAry = turls.split(Pattern.quote("|-|"));
      if (turlsAry != null)
      {
        String tLinkStr = conf.jt.itake("default.data_download", "tpl");
        for (int ti = 0; ti < turlsAry.length; ti ++)
        {
          String turlsAryStr = turlsAry[ti];
          if (!cls.isEmpty(turlsAryStr))
          {
            String[] turlsAryStrAry = turlsAryStr.split(Pattern.quote("|+|"));
            if (turlsAry != null)
            {
              if (turlsAryStrAry.length == 2)
              {
                tmpstr += tLinkStr;
                tmpstr = tmpstr.replace("{$text}", encode.htmlencode(turlsAryStrAry[0]));
                tmpstr = tmpstr.replace("{$url}", encode.htmlencode(turlsAryStrAry[1]));
              }
            }
          }
        }
  ...
  private String Module_Detail()
  {
    ...
    if (tArys != null)
    {
      ...
      tmpstr = tmpstr.replace("{$-download}", PP_FormatURLs((String)tDbc.getValue(tAry, "urls")));
      tmpstr = conf.jt.creplace(tmpstr);
    }
    return tmpstr;
  }
